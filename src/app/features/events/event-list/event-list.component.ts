...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatIconModule, MatButtonModule, MatProgressSpinnerModule, MatBadgeModule, EventCardComponent, EventFiltersComponent],
  template: `
    <div class="relative max-w-lg mx-auto h-[calc(100vh-6rem)] overflow-hidden">
      ...
      <div class="flex justify-between items-center mb-4 px-2">
        ...
        <button mat-icon-button (click)="toggleFilters()" class="text-text-primary bg-surface border border-white/10"
                [matBadge]="activeFiltersCount()" matBadgePosition="above after" matBadgeColor="warn" [matBadgeHidden]="activeFiltersCount() === 0">
      ...
      <!-- Filters Panel -->
      @if (showFilters()) {
        <div class="absolute inset-x-0 bottom-0 top-12 z-50 bg-background/95 backdrop-blur-3xl p-6 rounded-t-3xl border-t border-white/10 shadow-2xl transition-transform">
          <app-event-filters 
      ...
      @if (isLoading()) {
        ...
        <div class="h-[80vh] flex flex-col items-center justify-center text-center p-6 bg-surface/30 rounded-3xl border border-white/5 mx-2">
          ...
          <p class="text-text-secondary text-sm mb-6 max-w-[250px]">Prova a modificare i filtri o la tua ricerca per vedere più risultati.</p>
          <button mat-flat-button color="accent" class="!bg-accent text-white !rounded-full px-6 py-2" (click)="resetFilters()">
            Reimposta filtri
          </button>
        </div>
      } @else {
        <div class="h-[80vh] overflow-y-auto snap-y snap-mandatory scroll-smooth hide-scrollbar px-2"
          ...
          @for (event of events(); track event.id; let i = $index) {
            <div class="snap-start snap-always h-[80vh] pb-4 flex items-center">
              <app-event-card [event]="event" (cardClick)="viewDetail($event)"></app-event-card>
            </div>
          }
        </div>
        
        <!-- Pagination Dots -->
        <div class="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-10 pointer-events-none drop-shadow-lg">
          @for (event of events(); track event.id; let i = $index) {
            <div class="w-1.5 rounded-full transition-all duration-300"
                 [ngClass]="i === currentIndex() ? 'h-8 bg-accent shadow-[0_0_10px_rgba(255,107,53,0.8)]' : 'h-2 bg-white/30'"></div>
          }
        </div>
      }
    </div>
