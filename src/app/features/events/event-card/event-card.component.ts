...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatIconModule, MatChipsModule, NgOptimizedImage],
  animations: [cardExpandAnimation],
  host: { class: 'block w-full h-full cursor-pointer snap-center' },
  template: `
    <div class="relative w-full h-[85vh] rounded-3xl overflow-hidden shadow-2xl group"
         (click)="cardClick.emit(event.id)">
      
      <!-- Background Image -->
      <img [ngSrc]="event.coverImage" fill priority class="object-cover group-hover:scale-105 transition-transform duration-700" alt="Event cover">
      ...
      <div class="absolute top-10 left-4 right-4 flex justify-between items-start z-10">
        <div class="bg-surface/80 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 text-xs font-bold font-mono tracking-widest text-primary uppercase shadow-lg">
          {{ event.sport }}
        </div>
        @if (event.hasVip) {
          <div class="flex items-center bg-vip-gold/20 backdrop-blur-md px-3 py-1 rounded-full border border-vip-gold/50 shadow-[0_0_15px_rgba(255,215,0,0.3)] animate-pulse">
      ...
      <div class="absolute bottom-0 left-0 right-0 p-6 pb-12 z-10 flex flex-col justify-end">
        ...
        <!-- Coach Info -->
        @if (event.coach) {
          <div class="flex items-center mb-4 bg-white/5 backdrop-blur-sm p-3 rounded-2xl border border-white/10 w-fit">
        ...
        <div class="flex justify-between items-end">
          <div class="flex-1 mr-4">
             <div class="flex justify-between text-xs text-text-secondary mb-1">
               <span>Posti disponibili: {{ event.maxParticipants - event.currentParticipants }}</span>
               <span class="font-mono">{{ event.currentParticipants }}/{{ event.maxParticipants }}</span>
             </div>
             <div class="w-full bg-white/10 rounded-full h-2 overflow-hidden">
               <div class="bg-accent h-2 rounded-full transition-all duration-500" [style.width.%]="(event.currentParticipants / event.maxParticipants) * 100"></div>
             </div>
          </div>
          <div class="text-right">
             <div class="text-[10px] text-text-secondary uppercase tracking-widest mb-1">Costo</div>
             <div class="text-2xl font-black text-white bg-white/10 px-4 py-1 rounded-xl border border-white/20">
               €{{ event.cost }}
             </div>
          </div>
        </div>
...
export class EventCardComponent {
  @Input({ required: true }) event!: SportEvent;
