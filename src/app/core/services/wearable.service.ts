...
export class WearableService {
  ...
  startPolling(eventId: string, intervalSeconds: number): void {
    ...
    this.http.post<WearableSession>(`${environment.apiUrl}/wearable/sessions`, {
      eventId,
      ...
      this.pollingSubscription = interval(intervalSeconds * 1000)
        .pipe(
          ...
          switchMap(readings => this.http.post<{ saved: number }>(`${environment.apiUrl}/wearable/readings`, {
            sessionId: session.id,
            readings
          })),
          // After sending, we might want to refresh the session state or aggregated metrics
          switchMap(() => this.http.get<AggregatedMetrics>(`${environment.apiUrl}/wearable/sessions/${session.id}/aggregate`))
        )
        .subscribe(metrics => {
  ...
  stopPolling(eventId: string): void {
    ...
    if (currentSession) {
      this.http.patch<WearableSession>(`${environment.apiUrl}/wearable/sessions/${currentSession.id}`, {
        status: 'completed',
